 ...
 *
 *   Small Modification by Hunter Bond and Eric Vo
 *
   ...
   for (i = 0; i < grid_size[0]; i++) {
      ...
      for (j = 0; j < BLOCK_SIZE(i,grid_size[0],*n); j++) {
         ...
         for (k = 0; k < grid_size[1]; k++) {
            ...
            /* Determine the grid ID ofthe process getting
               the subrow */
...
/*
 *   Writes elements of a singly-subscripted array
 *   to a file.
 */

void write_subvector (
   char        *output_file,  /* IN - Output file */
   ...
   int          n)            /* IN - Array size */
{
   ...
   FILE *ofptr = fopen(output_file, "a");
   ...
   for (i = 0; i < n; i++) {
     if (dtype == MPI_DOUBLE)
        fwrite(&((double *)a)[i], get_size(dtype), 1, ofptr);
     else
      {
        if (dtype == MPI_FLOAT)
           fwrite(&((float *)a)[i], get_size(dtype), 1, ofptr);
        else if (dtype == MPI_INT)
           fwrite(&((int *)a)[i], get_size(dtype), 1, ofptr);
      }
   }
   fclose(ofptr);
}
...
/*
 *   Write a matrix distributed checkerboard fashion among
   ...
   for (i = 0; i < grid_size[0]; i++) {
      ...
      for (j = 0; j < BLOCK_SIZE(i,grid_size[0],m); j++) {
         ...
         if (!grid_id) {
            ...
            write_subvector (output_file, buffer, dtype, n);
